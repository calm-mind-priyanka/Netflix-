...
function syncInputs(){document.querySelectorAll('[data-path]').forEach(el=>{let v=get(settings,el.dataset.path);if(el.dataset.path==='files.fsub_channels'&&Array.isArray(v))v=v.join('\n');if(el.dataset.secret){el.value='';el.placeholder=(v?'Saved (leave blank to keep)':'');return}if(el.type==='checkbox')el.checked=Boolean(v);else el.value=v==null?'':v});const legacy=document.getElementById('legacyPreview');if(legacy)legacy.textContent=JSON.stringify(settings.ultron||{},null,2)}
